...
def train_and_save_models(pitcher_id):
   ...
   # ========================================================
    # 11. 점수 차 (투수팀 기준: 우리 팀 점수 - 상대 팀 점수)
    # ========================================================
    if (
        ...
        df['score_diff'] = np.where(
            ...
            df['home_score'] - df['away_score'],  # 이닝 초: 홈 투수 기준 (홈 - 어웨이)
            df['away_score'] - df['home_score']   # 이닝 말: 어웨이 투수 기준 (어웨이 - 홈)
        )
    else:
        df['score_diff'] = 0

    # ========================================================
    # 12. 필요한 컬럼 (가중치 계산을 위해 game_date 추가)
    # ========================================================
    req_cols = [
        'game_date', # <-- 시간 가중치 계산을 위해 유지
        'pitch_type',
        'zone_target',
        'stand',
        'balls',
    ...
    # ========================================================
    # ★ 추가된 부분: 시간 가중치 (Time Decay) 계산 ★
    # ========================================================
    max_date = X_train_full['game_date'].max()
    age_days = (max_date - X_train_full['game_date']).dt.days

    # 반감기 설정 (180일) - 필요시 튜닝 가능
    decay_parameter = 180 
    sample_weights = np.exp(-age_days / decay_parameter)

    # 머신러닝 입력을 위해 불필요해진 날짜 컬럼 제거
    X_train = X_train_full.drop(columns=['game_date'])
    X_test = X_test_full.drop(columns=['game_date'])

    # ========================================================
    # 16. 구종 예측 Random Forest (가중치 적용)
    # ========================================================
    ...
    # fit 함수에 sample_weight 파라미터 추가
    clf_pitch.fit(X_train, y_pitch_train, sample_weight=sample_weights)

    # ========================================================
    # 17. 코스 예측 Random Forest (가중치 적용)
    # ========================================================
    ...
    # fit 함수에 sample_weight 파라미터 추가
    clf_zone.fit(X_train, y_zone_train, sample_weight=sample_weights)
    ...
    # ========================================================
    # 22. 모델 정보 저장 (columns에 game_date 제외)
    # ========================================================
    model_data = {
        'pitch_model': clf_pitch,
        'zone_model': clf_zone,
        'columns': list(X_train.columns), # game_date 제외된 컬럼 저장
